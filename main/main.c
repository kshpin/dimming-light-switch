...
static void on_off_button_pressed(void)
{
    ...
    ESP_LOGI(TAG, "Manual control button pressed, new state: %s", new_state ? "ON" : "OFF");
...
{
    ESP_LOGI(TAG, "Zigbee On/Off updated, new state: %s", on ? "ON" : "OFF");
...
{
    ESP_LOGI(TAG, "Zigbee Level updated, new level: %d", level);
